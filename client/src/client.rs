...
use crate::{bellscoin, deserialize_hex};
use bitcoin_private::hex::exts::DisplayHex;
use jsonrpc;
use serde;
use serde_json;

use crate::bellscoin::address::{NetworkChecked, NetworkUnchecked};
use crate::bellscoin::hashes::hex::FromHex;
use crate::bellscoin::secp256k1::ecdsa::Signature;
use crate::bellscoin::{
...
impl<'a> RawTx for &'a Transaction {
    fn raw_hex(self) -> String {
        bellscoin::consensus::encode::serialize_hex(self)
    }
}
...
pub trait RpcApi: Sized {
    ...
    fn get_block(&self, hash: &bellscoin::BlockHash) -> Result<Block> {
        let hex: String = self.call("getblock", &[into_json(hash)?, 0.into()])?;
        deserialize_hex(&hex)
    }

    fn get_block_hex(&self, hash: &bellscoin::BlockHash) -> Result<String> {
        self.call("getblock", &[into_json(hash)?, 0.into()])
    }

    fn get_block_info(&self, hash: &bellscoin::BlockHash) -> Result<json::GetBlockResult> {
        self.call("getblock", &[into_json(hash)?, 1.into()])
    }
    //TODO(stevenroose) add getblock_txs

    fn get_block_header(&self, hash: &bellscoin::BlockHash) -> Result<bellscoin::block::Header> {
    ...
    fn get_best_block_hash(&self) -> Result<bellscoin::BlockHash> {
        self.call("getbestblockhash", &[])
    }

    /// Get block hash at a given height
    fn get_block_hash(&self, height: u64) -> Result<bellscoin::BlockHash> {
    ...
    fn get_raw_transaction(
        &self,
        txid: &bellscoin::Txid,
        block_hash: Option<&bellscoin::BlockHash>,
    ) -> Result<Transaction> {
    ...
    fn get_block_filter(
        &self,
        block_hash: &bellscoin::BlockHash,
    ...
    fn list_since_block(
        &self,
        blockhash: Option<&bellscoin::BlockHash>,
    ...
    ) -> Result<Vec<bellscoin::BlockHash>> {
    ...
    fn generate(&self, block_num: u64, maxtries: Option<u64>) -> Result<Vec<bellscoin::BlockHash>> {
        self.call("generate", &[block_num.into(), opt_into_json(maxtries)?])
    }

    /// Mark a block as invalid by `block_hash`
    fn invalidate_block(&self, block_hash: &bellscoin::BlockHash) -> Result<()> {
        self.call("invalidateblock", &[into_json(block_hash)?])
    }

    /// Mark a block as valid by `block_hash`
    fn reconsider_block(&self, block_hash: &bellscoin::BlockHash) -> Result<()> {
    ...
    fn get_raw_mempool(&self) -> Result<Vec<bellscoin::Txid>> {
    ...
    fn get_raw_mempool_verbose(
        &self,
    ) -> Result<HashMap<bellscoin::Txid, json::GetMempoolEntryResult>> {
        self.call("getrawmempool", &[into_json(true)?])
    }

    /// Get mempool data for given transaction
    fn get_mempool_entry(&self, txid: &bellscoin::Txid) -> Result<json::GetMempoolEntryResult> {
    ...
    fn send_raw_transaction<R: RawTx>(&self, tx: R) -> Result<bellscoin::Txid> {
    ...
    fn wallet_process_psbt(
        ...
        let defaults = [
            true.into(),
            into_json(json::SigHashType::from(bellscoin::sighash::EcdsaSighashType::All))?,
            true.into(),
        ];
    ...
    /// Submit a block
    fn submit_block(&self, block: &bellscoin::Block) -> Result<()> {
        let block_hex: String = bellscoin::consensus::encode::serialize_hex(block);
...
mod tests {
    ...
    #[test]
    fn test_raw_tx() {
        use crate::bellscoin::consensus::encode;
        let client = Client::new("http://localhost/".into(), Auth::None).unwrap();
        let tx: bellscoin::Transaction = encode::deserialize(&Vec::<u8>::from_hex("").unwrap()).unwrap();
