...
impl<C: RpcApi> Queryable<C> for bellscoin::block::Block {
    type Id = bellscoin::BlockHash;

    fn query(rpc: &C, id: &Self::Id) -> Result<Self> {
        let rpc_name = "getblock";
        let hex: String = rpc.call(rpc_name, &[serde_json::to_value(id)?, 0.into()])?;
        let bytes: Vec<u8> = bellscoin::hashes::hex::FromHex::from_hex(&hex)?;
        Ok(bellscoin::consensus::encode::deserialize(&bytes)?)
    }
}

impl<C: RpcApi> Queryable<C> for bellscoin::transaction::Transaction {
    type Id = bellscoin::Txid;
