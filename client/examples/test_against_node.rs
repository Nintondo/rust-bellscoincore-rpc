...
//! Core node.
extern crate bellscoincore_rpc;

use bellscoincore_rpc::{bellscoin, Auth, Client, Error, RpcApi};
...
fn main_result() -> Result<(), Error> {
    ...
    let bellscoin_block: bellscoin::Block = rpc.get_by_id(&best_block_hash)?;
    println!("best block hash by `get`: {}", bellscoin_block.header.prev_blockhash);
    let bellscoin_tx: bellscoin::Transaction = rpc.get_by_id(&bellscoin_block.txdata[0].txid())?;
    println!("tx by `get`: {}", bellscoin_tx.txid());
